...
gachi_query = [["0","만약 초능력이 생긴다면 무엇을 하실 건가요"], ["1","레몬메론에서 만난 사람이 친한 친구의 전 애인이라면 어떻게 하실 건가요"], ["2","로또에 당첨된다면 무엇을 하실 건가요"]]
favor_query = [["0","레몬", "메론"],["1","소주", "맥주"],["2","켄즈", "핀토스"],["3","치킨", "피자"],["4","외향적", "내향적"],["5","축구", "야구"],["6","노른자", "흰자"],["7","트와이스", "레드벨벳"],["8","칙촉", "홈런볼"],["9","버거킹", "맘스터치"],["10","ACK", "SYNACK"],["11","태진아", "엑소"],["12","노래방가서 스마트폰 만지기", "노래방에서 노래부르기"],["13","그랑", "책다방"],["14","롯데리아", "서브웨이"],["15","ㅇㅅㅇ", "ㅇㅂㅇ"],["16","왕비성", "닭칼"],["17","오라우", "두메"],["18","폰노이만", "아인슈타인"],["19","서울 되기", "소울 되기"]]
...
def login_sent(request):
    try:
        ...
        return render(request, 'dating/error.html',{'error_message':"아이디가 없는데용????????"})
...
def match(request):
    ...
    disab=""
    if match_found==None:
        return render(request, 'dating/error_match.html',{'error_message':"더 이상 사람이 없습니다 까비!", 'my_user':my_user})
